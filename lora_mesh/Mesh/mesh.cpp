...
void initMesh(MeshEvents_t *events, int numOfNodes)
{
	...
	if (nodesMap == NULL)
	{
		Serial.println("Could not allocate memory for nodes map");
	}
	else
	{
		Serial.println("Memory for nodes map is allocated");
	}
	...
	// if (namesMap == NULL)
	// {
	// 	Serial.println("Could not allocate memory for names map");
	// }
	// else
	// {
	// 	Serial.println("Memory for names map is allocated");
	// }
	...
	if (sendQueue == NULL)
	{
		Serial.println("Could not create send queue!");
	}
	else
	{
		Serial.println("Send queue created!");
	}
	...
	{
		Serial.println("Could not create LoRa message queue!");
	}
	else
	{
		Serial.println("LoRa message queue created!");
	}
...
void meshTask(void *pvParameters)
{
	...
	while (1)
	{
		...
		if ((millis() - notifyTimer) >= syncTime)
		{
			...
			{
				Serial.println("Checking mesh map");
				if (!cleanMap())
				{
				...
				Serial.println("Sending mesh map");
			...
			{
				Serial.println("Cannot access map for clean up and syncing");
			}
		}
		...
		{
			Serial.println("Switching sync time to DEFAULT_SYNCTIME");
		...
		if ((loraState == MESH_TX) && ((millis() - txTimeout) > 7500))
		{
			...
			loraState = MESH_IDLE;
			Serial.println("loraState stuck in TX for 2 seconds");
		}
...
void OnRxDone(uint8_t *rxPayload, uint16_t rxSize, int16_t rxRssi, int8_t rxSnr)
{
	...
	if ((rxBuffer[0] == 'L') && (rxBuffer[1] == 'o') && (rxBuffer[2] == 'R'))
	{
		...
		{
			Serial.println("Got map message");
			// Mapping received
			...
			{
				Serial.print("Invalid map, end marker is missing from ");
				Serial.println(thisMsg->from);
			...
			{
				Serial.println("Could not access map to add node");
			}
		}
		else if (thisDataMsg->type == LORA_DIRECT)
		{
			if (thisDataMsg->dest == deviceID)
			{
				...
				Serial.print("Got data message type ");
				Serial.print(thisDataMsg->data[0]);
				Serial.print(" >");
				Serial.print((char *)&thisDataMsg->data[1]);
				Serial.println("<");
			...
			if (thisDataMsg->dest == deviceID)
			{
				...
				if (xSemaphoreTake(accessNodeList, (TickType_t)1000) == pdTRUE)
				{
					if (getRoute(thisDataMsg->from, &route))
					{
						...
						if (route.firstHop == 0)
						{
							Serial.print("Route for ");
							Serial.print(route.nodeId);
							Serial.println(" is direct");
						...
						{
							Serial.println("Cannot forward message because send queue is full");
						}
					}
					else
					{
						Serial.print("No route found for ");
						Serial.println(thisMsg->from);
					}
					xSemaphoreGive(accessNodeList);
				}
				...
				{
					Serial.println("Could not access map to forward package");
				}
			}
			else
			{
		...
		else if (thisDataMsg->type == LORA_BROADCAST)
		{
			...
			Serial.print("Handling broadcast with ID ");
			Serial.print(thisDataMsg->dest);
			Serial.print(" from ");
			Serial.println(thisDataMsg->from);

			// Check if this broadcast is coming from ourself
			if ((thisDataMsg->dest & 0xFFFFFF00) == (deviceID & 0xFFFFFF00))
			{
				Serial.println("We received our own broadcast, dismissing it");
				return;
			}
			// Check if we handled this broadcast already
			if (isOldBroadcast(thisDataMsg->dest))
			{
				Serial.println("Got an old broadcast, dismissing it");
				return;
			}
			...
			if (!addSendRequest(thisDataMsg, tempSize))
			{
				Serial.println("Cannot forward broadcast because send queue is full");
			}

			// This is a broadcast, call user callback to handle the data
			Serial.print("Got data broadcast ");
			Serial.println((char *)thisDataMsg->data);
...
void OnTxDone(void)
{
	Serial.println("LoRa send finished");
	loraState = MESH_IDLE;
...
{
	Serial.println("LoRa TX SW timer timeout");
	loraState = MESH_IDLE;
...
{
	Serial.println("LoRa RX SW timer timeout");
	if (loraState != MESH_TX)
	{
		loraState = MESH_IDLE;
...
void OnCadDone(bool cadResult)
{
	...
	if (cadResult)
	{
		...
		{
			Serial.print("CAD returned channel busy ");
			Serial.print(CAD_RETRY);
			Serial.println(" times, giving up");
	...
	{
		Serial.println("CAD returned channel free");
...
bool addSendRequest(dataMsg *package, uint8_t msgSize)
{
	if (sendQueue != NULL)
	{
		...
		if (next != SEND_QUEUE_SIZE)
		{
			...
			{
				Serial.println("Send queue is busy");
		...
		{
			Serial.println("Send queue is full");
	...
	{
		Serial.println("Send queue not yet initialized");
		return false;
	}
}
