...
boolean addNode(uint32_t id, uint32_t hop, uint8_t hopNum)
{
	...
	for (int idx = 0; idx < _numOfNodes; idx++)
	{
		if (nodesMap[idx].nodeId == id)
		{
			if (nodesMap[idx].firstHop == 0)
			{
				...
				Serial.print("Node ");
				Serial.print(_newNode.nodeId);
				Serial.println(" already exists as direct");
			...
			else
			{
				if (hop == 0)
				{
					...
					Serial.print("Node ");
					Serial.print(_newNode.nodeId);
					Serial.println(" removed because it was a sub");
				...
				else
				{
					...
					if (nodesMap[idx].numHops < hopNum)
					{
						...
						Serial.print("Node ");
						Serial.print(_newNode.nodeId);
						Serial.println(" exist with a lower number of hops");
					...
					else
					{
						...
						Serial.print("Node ");
						Serial.print(_newNode.nodeId);
						Serial.println(" exist with a higher number of hops");
	...
	Serial.print(id);
	Serial.print(" with hop ");
	Serial.print(hop);
	Serial.print(" and num hops ");
	Serial.print(hopNum);
...
void clearSubs(uint32_t id)
{
	for (int idx = 0; idx < _numOfNodes; idx++)
	{
		if (nodesMap[idx].firstHop == id)
		{
			...
			Serial.print(nodesMap[idx].nodeId);
			Serial.print(" with hop ");
			Serial.print(nodesMap[idx].firstHop);
...
bool cleanMap(void)
{
	...
	for (int idx = 0; idx < _numOfNodes; idx++)
	{
		...
		if (((millis() > (nodesMap[idx].timeStamp + inActiveTimeout))) || (nodesMap[idx].numHops > 48))
		{
			...
			Serial.print(" with hop ");
			Serial.print(nodesMap[idx].firstHop);
			Serial.print(" timed out or has too many hops");
